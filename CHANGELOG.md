...
- **B52** – `src/views/InscripcionView.jsx`: Corrección de pantalla en blanco en paso de pago.
...
- **B62** – `src/views/InscripcionView.jsx`: Inicialización automática de `cursoId` al cargar cursos.
- **B41** – `src/components/ModalPIN.jsx`: Corregido texto prematuro de éxito.
- **B13** – `src/views/InscripcionView.jsx` y `src/services/ReservaService.js`: Reemplazo de campo "edad" por "fechaNacimiento".
- **B18** – `src/views/InscripcionView.jsx`: Paso 1 dividido en 1a (Configurar Clase) y 1b (Selección de Horario).
...
- **B26** – `src/views/InscripcionView.jsx`: Tarjetón de resumen de clase en paso 1b.
- **B29** – `src/views/InscripcionView.jsx`: Corregido z-index de la moto en la barra de progreso.
- **B30** – `src/views/InscripcionView.jsx`: Agregado icono `MapPin` al Select de Estado.
- **B31** – `src/views/InscripcionView.jsx`: Cambiado icono de Cédula a `Contact`.
- **B33** – `src/views/InscripcionView.jsx`: Agregado placeholder "Ej: Petare" al campo Zona.
- **B36** – `src/views/InscripcionView.jsx`: Cambiado icono de Banco Emisor a `Building2` (luego a `CreditCard`).
...
- **B48** – `src/views/InscripcionView.jsx`: Ajustes de padding para recuperar espacio vertical.
- **B47** – `src/components/UI.jsx`: Cambiado `transition-all` a `transition-colors` para eliminar vibración.
- **B14, B46, B60** – `src/views/PortalEstudiante.jsx`, `src/views/EstudiantePanel.jsx`: Correcciones de sesión y acceso al panel.

### Documentación Creada/Actualizada
- `BACKLOG_V2.md`: Lista completa de deuda técnica.
- `BITACORA_EXPERIENCIAL.md`: Decisiones clave y lecciones aprendidas.
- `ARRANQUE_MEMORIA_IA.md`: Contexto para restaurar la memoria del Arquitecto.
...
- **B5** – `src/context/AppContext.jsx`: Corrección de `appId` a `motoescuela-pro-v1`.
- **B69** – `BACKLOG_V2.md`: Registrada deuda para habilitar/deshabilitar bloques de horario.
- **B70** – `BACKLOG_V2.md`: Registrada deuda para agrupar y deshabilitar bloques por grupo.

### Sesión 17/06/2026 (madrugada) – Corrección del Dashboard
- **B51** – `src/context/AppContext.jsx`: Agregada función `handleSaveInstructorSeguro`.
- **B71-B74** – `BACKLOG_V2.md`: Documentadas funciones ausentes como deuda técnica.
...
**Bugs cerrados:**
- B51: Implementación de `StaffService.js` con REST API.
...
- C1: `fechaNacimiento` agregado a `CAMPOS_PERMITIDOS`.
...
- HomeView, LoginView, Paso 4, Calendario, Disponibilidad, Reloj SVG, Placeholders.

---
### Sesión 19/06/2026 – Correcciones finales y mejoras (v1.7.8)

**Inscripción:**
- Corregida validación de recursos para `traeMoto === 'Sí'`.
- Lock extendido 10 minutos al avanzar al paso 4.
- Protegido `useEffect` de cambio de fecha.
- Agregada función `formatearFechaNatural`.

**Dashboard:**
- Acordeones colapsables, selectores de moneda, grid de 2 columnas, toggle de descuento.

**Pendiente:**
- `buscarProximaFechaDisponible` debe buscar siempre hacia adelante.
...
- Placeholders, validaciones y atributos HTML.
...
- Modo corrección con footer limitado.
