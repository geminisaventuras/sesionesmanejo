// @build: 2026-06-22 | id: CONFIG-HUB-INDEPENDIENTE | desc: Hub de configuración como página independiente
...
const header = <DashboardHeader title="Configuración" onBack={() => navigate('/dashboard')} onLogout={handleLogout} notifications={notifications} />;  const footer = <DashboardFooter
    tabs={footerTabs}
    activeTab="config"
    onTabChange={(id) => {
    ...
    <AppShell header={header} footer={footer} bgColor="bg-gray-50">
      <div className="p-4 space-y-6">
        <div className="grid grid-cols-2 gap-3">
          <button type="button" onClick={() => navigate('/admin/config/cursos')} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center gap-2 hover:border-blue-300 transition-colors">
            <BookOpen size={28} className="text-blue-500" /><span className="font-bold text-sm text-gray-800">Cursos</span>
          </button>
          <button type="button" onClick={() => navigate('/admin/config/sedes')} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center gap-2 hover:border-blue-300 transition-colors">
            <MapPin size={28} className="text-blue-500" /><span className="font-bold text-sm text-gray-800">Sedes</span>
          </button>
          <button type="button" onClick={() => navigate('/admin/config/horarios')} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center gap-2 hover:border-blue-300 transition-colors">
            <Clock size={28} className="text-blue-500" /><span className="font-bold text-sm text-gray-800">Horarios</span>
          </button>
          <button type="button" onClick={() => navigate('/admin/config/motos')} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center gap-2 hover:border-blue-300 transition-colors">
            <Bike size={28} className="text-blue-500" /><span className="font-bold text-sm text-gray-800">Motos</span>
          </button>
          <button type="button" onClick={() => navigate('/admin/config/instructores')} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center gap-2 hover:border-blue-300 transition-colors">
            <Users size={28} className="text-blue-500" /><span className="font-bold text-sm text-gray-800">Instructores</span>
          </button>
          <button type="button" onClick={() => navigate('/admin/config/proveedores')} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center gap-2 hover:border-blue-300 transition-colors">
