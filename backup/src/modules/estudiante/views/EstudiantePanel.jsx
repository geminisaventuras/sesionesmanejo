// @build: 2026-06-22 | id: ESTUDIANTE-SERVICIOS | desc: Panel del estudiante usando ReservaService y CursoService (sin acceso directo a Firestore)
...
export function EstudiantePanel() {
  ...
  useEffect(() => {
    ...
    const obtenerReserva = async () => {
      try {
        const result = await ReservaService.obtenerReservaPorUsuario(uid);
        if (!cancel && result.success && result.data) {
          setLocalReserva(result.data);
        } else if (!cancel && !result.success) {
          showToast('Error al buscar tus reservas: ' + (result.error?.message || 'fallo'), 'error');
        }
      } finally {
  ...
  const buscarCurso = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (!result.success) showToast('Error al cargar el curso: ' + (result.error?.message || 'fallo'), 'error');
      }
    } catch (e) {
      setCursoDirecto(null);
      setBusquedaCursoFallida(true);
    }
  }, [reservaContext, localReserva, showToast]);
