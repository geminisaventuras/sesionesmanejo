// @build: 2026-06-23.CENTINELA | id: TIMER-CORREGIDO | desc: Temporizador con 4 correcciones del Centinela: bug de estado, precisión de pausa, carrera de Date.now() y heartbeat de presencia.
...
export function useSessionTimer(reservaId, esInstructor, saveReserva, showToast, opcionesRef) {
  ...
  // ── Heartbeat de presencia del estudiante (CORRECCIÓN 4) ──
  useEffect(() => {
  ...
  const derivados = useMemo(() => {
    ...
    // ✅ CORRECCIÓN 2: el reduce ahora suma duracionSegundos si existe, o convierte duracion (minutos) a segundos
  ...
  // ✅ CORRECCIÓN 1: pausaActiva toma el valor de localPausaActiva
  const sgta = useMemo(() => ({
    ...SGTA_DEFAULTS,
    ...derivados,
  ...
  // ✅ CORRECCIÓN 3: se usa const ahora = Date.now() una sola vez para estado local y Firestore
  ...
  // ✅ CORRECCIÓN 2: cálculo de duración en segundos, guardado como duracionSegundos
  const reanudarSesion = useCallback(async () => {
    ...
    if (pausaInicio) {
      ...
      const pausas = [...(reserva.pausas || []), { inicio: pausaInicio, fin: Date.now(), motivo: sgtaRef.current.pausaMotivo, duracionSegundos: durSegundos }];
