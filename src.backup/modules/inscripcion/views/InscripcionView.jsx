...
const disponibilidad = useMemo(() => {
  ...
  const handleConfirmarPago = useCallback(async () => {
    ...
    const result = await ReservaService.crearReserva({
      ...
      fechaNacimiento, pagoTotalMoneda: baseUSD, pagoTotalVES: parseFloat(precioFinalVES),terminosAceptados: true,
  ...
  .filter(i => !(ctx.ocupacionConfirmada || []).some(r => {
    if (r.estadoPago !== 'Pendiente' && r.estadoPago !== 'Aprobado') return false;
    if (r.instructorId !== i.id) return false;
    if (String(r.horaId) !== String(bloque.id)) return false;
    return r.fecha === form.fecha1 || r.fecha === disponibilidad?.fecha2Calc || r.fecha2 === form.fecha1 || r.fecha2 === disponibilidad?.fecha2Calc;
  }));
   let motosLibres = [];
if (necesitaMoto) {
  motosLibres = (ctx.motos || [])
    ...
    .filter(m => !(ctx.ocupacionConfirmada || []).some(r => {
      if (r.estadoPago !== 'Pendiente' && r.estadoPago !== 'Aprobado') return false;
  ...
  const handleNext = async () => {
      ...
      if (!ctx.fbUser) {
        ...
        if (result.success) {
          ...
          showToast('Esta cédula ya está registrada. Si crees que es un error, contacta al administrador.', 'error');
