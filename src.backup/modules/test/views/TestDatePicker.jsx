...
/* ---------------------------------------------------------------
 * COMPONENTE SelectorColumna (versión funcional con diseño mejorado)
 * ---------------------------------------------------------------
 */
...
/* ---------------------------------------------------------------
 * PÁGINA DE PRUEBA TestDatePicker
 * ---------------------------------------------------------------
 */
const meses = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'];

const getMesAbrev = (mesStr) => {
  return meses[parseInt(mesStr) - 1] || 'Ene';
};
...
export default function TestDatePicker() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center">
      ...
      >
        Abrir selector ({`${tempFecha.dia}/${tempFecha.mes}/${tempFecha.ano}`})
      </button>
        ...
        <div className="fixed inset-0 z-50 bg-white flex flex-col">
          ...
          <div className="flex-1 flex gap-0 px-4 min-h-0">
            <div className="flex-1 relative">
              <SelectorColumna
                items={dias}
                selected={tempFecha.dia}
                onSelect={(val) => setTempFecha((prev) => ({ ...prev, dia: val }))}
              />
            </div>
            ...
            <div className="flex-1 relative">
              <SelectorColumna
                items={meses}
                selected={getMesAbrev(tempFecha.mes)}
                onSelect={(val) => {
                  const idx = meses.indexOf(val);
                  setTempFecha((prev) => ({ ...prev, mes: String(idx + 1).padStart(2, '0') }));
                }}
              />
            </div>
            {/* Separador vertical pronunciado */}
            ...
            <div className="flex-1 relative">
              <SelectorColumna
                items={anos}
                selected={tempFecha.ano}
                onSelect={(val) => setTempFecha((prev) => ({ ...prev, ano: val }))}
              />
            </div>
          </div>
