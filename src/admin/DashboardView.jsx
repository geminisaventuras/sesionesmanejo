...
const CRUDView = memo(({ titulo, items, saveFn, formComponent: FormComponent, setTab }) => {
  ...
  const handleSaveGeneral = useCallback(async (datos, id = null) => {
    if (!isAdmin) return;
    if (!id && (saveFn === saveInstructor || saveFn === saveProveedor) && datos.email && datos.password) {
      try { await createStaffUser(datos.email, datos.password, saveFn === saveInstructor ? 'instructor' : 'proveedor', datos); setItemEdit(null); showToast('Usuario creado correctamente en el sistema'); return; }
      catch (error) { showToast(error.code === 'auth/email-already-in-use' ? 'Este correo ya está registrado en el sistema' : 'Error al crear el usuario: ' + error.message, 'error'); return; }
    }
    if (saveFn === saveInstructor && datos.esPrincipal) { for (let i of (instructores || [])) if (String(i.id) !== String(id) && i.esPrincipal) await saveInstructor({ ...i, esPrincipal: false }); }
    if (typeof saveFn === 'function') await saveFn(datos);
    setItemEdit(null); showToast('Guardado exitoso');
  }, [isAdmin, saveFn, createStaffUser, instructores, saveInstructor, showToast]);
