...
const evaluarDisponibilidad = ({
  form, horarios, instructores, motos, reservasConfirmadas, activeLocks, selectingBlockId, currentUserId, todayStr, lockId, bloqueosProveedor, cursoUnDia
}) => {
  ...
  const reservasDeOtros = (reservasConfirmadas || []).filter(r => r.userId !== currentUserId);
...
const disponibilidad = useMemo(() => {
 ...
 return evaluarDisponibilidad({
    ...
    currentUserId: ctx.fbUser?.uid, todayStr, lockId, bloqueosProveedor, cursoUnDia
  });
}, [form, ctx.horarios, ctx.instructores, ctx.motos, ctx.ocupacionConfirmada, activeLocks, selectingBlockId, ctx.fbUser?.uid, todayStr, lockId, clockTick, bloqueosProveedor, cursoUnDia]);
  useEffect(() => {
  ...
  const handleSelectHorario = useCallback(async (bloque) => {
    ...
    if (!ctx.fbUser) { showToast('Espera un momento...', 'error'); return; }
      ...
      return true;
        ...
        const reservasDeOtros = (ctx.ocupacionConfirmada || []).filter(r => r.userId !== ctx.fbUser?.uid);
    console.log('🔍 [DEBUG userIds]', {
      ...
      ocupacionConfirmada: ctx.ocupacionConfirmada,
    });
    const necesitaMoto = form.traeMoto !== 'Sí';
