...
import { paso1Schema, paso4Schema } from '../../shared/schemas/validations';
...
export const InscripcionView = () => {
  ...
  const handleNext = async () => {
    if (step === '1') {
      // Validación con Zod
      const resultado = paso1Schema.safeParse({
        nombre: form.nombre,
    ...
    if (step === '4') {
      ...
      const resultadoPago = paso4Schema.safeParse({
      ...
      if (!resultadoPago.success) {
        const mensajes = resultadoPago.error.issues.map(i => i.message).join('\n');
        showToast(mensajes, 'error');
        return;
      }
