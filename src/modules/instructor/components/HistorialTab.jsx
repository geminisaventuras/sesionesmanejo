  ...
  const reservasHistoricas = useMemo(() => {
    return reservas
      .filter(r => {
        ...
        const esCancelada = r.estadoPago === 'Cancelado' || r.estadoCurso === 'Cancelado';
        const esRechazada = r.estadoPago === 'Rechazado' || r.estadoCurso === 'Rechazado';
        return esPasada || esCompletada || esCancelada || esRechazada;
      })
      .sort((a, b) => {
      ...
      {reservasVisibles.length === 0 ? (
        ...
        <div className="space-y-2">
          {reservasVisibles.map(r => {
            ...
            if (r.estadoPago === 'Cancelado' || r.estadoCurso === 'Cancelado') badges.push('CANCELADA');
            if (r.estadoPago === 'Rechazado' || r.estadoCurso === 'Rechazado') badges.push('RECHAZADA');
