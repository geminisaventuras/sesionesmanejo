...
export default function ResumenTab({
  ...
  const completadas = useMemo(() => reservas.filter(r => r.estadoCurso === 'Aprobado'), [reservas]);
  const ordenadas = useMemo(() => ordenarReservasPorFechaHora(activas, horarios), [activas, horarios]);
  const grupos = useMemo(() => agruparReservasPorPeriodo(ordenadas, hoy), [ordenadas, hoy]);

  const clasesHoy = grupos.hoy.length;
  const ultimasCompletadas = useMemo(() => [...completadas].reverse().slice(0, 3), [completadas]);

  if (cargando) {
    return (
      <div className="space-y-4" aria-busy="true">
        <div className="h-24 bg-blue-100 rounded-3xl animate-pulse"></div>
        <div className="space-y-3">
          {[1, 2, 3, 4].map(i => <ReservaCardSkeleton key={i} />)}
        </div>
      </div>
    );
  }
