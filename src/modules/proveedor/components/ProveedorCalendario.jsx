...
const ProveedorCalendario = () => {
  const { user, motos, ocupacionConfirmada } = useContext(AppContext);
  const { showToast } = useToast();
  const { bloqueos, loading, invalidarCache } = useProveedorBloqueos(user?.uid);
  ...
  // Reservas activas de las motos del proveedor (usando ocupacionConfirmada)
  ...
  // Sincronizar los bloqueos del día seleccionado
  useEffect(() => {
    if (!fecha) {
      setBloqueosDia([]);
      return;
