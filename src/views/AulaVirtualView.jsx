// @build: 2026-06-20.16-00-00 | id: AULA-MODAL-CONFIRM | desc: Modal de confirmación personalizado, mostrarBarra en módulos excedidos
...
import { alertas } from '../modules/shared/utils/alertas';
...
useEffect(() => { alertas.inicializar(); }, []);
  const { reserva, sgta, modalConfirmacion, toggleModulo, pausarSesion, reanudarSesion } = useSessionTimer(reservaId, esInstructor, saveReserva, showToast);
  ...
  return (
    <AppShell header={header} footer={footer}>
      <div className="p-4 space-y-3">
        ...
        {esInstructor && !todosCompletados && sgta.generalActivo && !sgta.pausaActiva && (<Button type="button" onClick={() => setMostrarSelectorPausa(true)} variant="outline" className="!py-2 !text-xs" icon={Pause}>Pausar Sesión</Button>)}
        ...
        {modulosPendientes.length > 0 && (
          <div className="space-y-1.5">
            {modulosPendientes.map((mod, i) => {
              ...
              const esModuloActivo = sgta.moduloEnProgreso === nombreModulo && (sgta.moduloActivo || sgta._moduloExcedido);
              const esPrimerPendiente = i === 0;
              const interactuable = esInstructor && esPrimerPendiente && !sgta.pausaActiva;
              let estado = esPrimerPendiente ? (sgta._moduloExcedido && sgta.moduloEnProgreso === nombreModulo ? 'excedido' : (esModuloActivo ? 'activo' : 'pendiente')) : 'bloqueado';
              if (sgta._moduloExcedido && sgta.moduloEnProgreso === nombreModulo) estado = 'excedido';
              const pctTiempo = esModuloActivo ? Math.min(100, ((sgta.moduloSegundos || 0) / (duracionModulo * 60)) * 100) : 0;
              const esReceso = esModuloActivo && (duracionModulo * 60 - (sgta.moduloSegundos || 0)) <= 300 && !sgta._moduloExcedido;
              return (<ModuloItem key={i} nombre={nombreModulo} duracion={duracionModulo} estado={estado} onClick={() => interactuable && toggleModulo(nombreModulo)} disabled={!interactuable} mostrarBarra={esModuloActivo} progreso={esModuloActivo ? pctTiempo : 0} tiempoActual={esModuloActivo ? sgta.moduloSegundos || 0 : 0} esReceso={esReceso} tiempoExcedido={sgta._moduloExcedido && sgta.moduloEnProgreso === nombreModulo} />);
            })}
          </div>
        )}
            ...
            {!mostrandoInputOtro ? (
              <>
                ...
                <div className="space-y-2">
                  {MOTIVOS_PAUSA.map(m => (
                    <button key={m.id} onClick={() => { if (m.id === 'otro') setMostrandoInputOtro(true); else { pausarSesion(m.label); setMostrarSelectorPausa(false); } }} className="w-full flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-colors text-left">
      ...
      {mostrarMaterial && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-5 max-h-[80vh] overflow-y-auto">
            ...
            <div className="flex gap-2 mb-4">{['senales','leyes','glosario'].map(s => (<button key={s} onClick={() => setSeccionMaterial(s)} className={`px-3 py-1.5 rounded-full text-xs font-bold ${seccionMaterial === s ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>{s.charAt(0).toUpperCase() + s.slice(1)}</button>))}</div>
            <div className="space-y-3">{MATERIAL_APOYO[seccionMaterial]?.map((item, i) => (<div key={i} className="bg-gray-50 p-3 rounded-xl"><p className="font-bold text-sm text-gray-900">{item.nombre || item.titulo || item.termino}</p><p className="text-xs text-gray-600">{item.descripcion || item.texto || item.definicion}</p></div>))}</div>
          </div>
        </div>
      )}
    </AppShell>
