// @build: 2026-06-16.02-45-00 | id: B44-B60-B61 | desc: Muestra cursoId en diagnóstico y botón de reintentar
...
export const EstudiantePanel = () => {
  const { reservas, cursos, horarios, instructores, sedes, user, fbUser, showToast, saveReserva, logoutUser } = useContext(AppContext);
  ...
  // Obtener reserva desde Firestore si no está en contexto
  useEffect(() => {
    if (!uid) return;
    if (reservaContext) {
  ...
  // Buscar curso en Firestore usando el cursoId de la reserva
  ...
  return (
    <div className="p-5 pb-24 bg-gray-50 min-h-screen">
      <div className="flex items-center gap-3 mb-6">
        <button type="button" onClick={handleLogout} className="p-2 bg-white rounded-full shadow text-gray-500"><LogOut size={20} /></button>
